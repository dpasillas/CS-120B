/* dpasi001_lab8_part4.c - Feb 12, 2013
...
uchar LI_Tick(uchar LI_State) {
	...
	case LI_PositionCursor:
		...
		break;
	case LI_DisplayChar:
		if (LCD_write_str && i<16)
			LI_State = LI_PositionCursor, ++i;
		else 
			LI_State = LI_WaitGo0;
		LCD_WriteDataEnd();
		break;
	...
	case LI_Clr:
		...
		break;
	case LI_PositionCursor:
		LCD_Cursor((LCD_write_str)?i:LCD_char_pos);
		break;
	case LI_DisplayChar:
