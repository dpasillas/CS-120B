...
// The majority of this file was provided for lab 8
...
unsigned char *LCD_Data_PROXY = &PORTD_PROXY;
unsigned char *LCD_Ctrl_PROXY = &PORTA_PROXY;
...
void LCD_WriteCmdStart(unsigned char cmd) {
	*LCD_Ctrl_PROXY = SetBit(*LCD_Ctrl_PROXY,LCD_RS, 0);
	*LCD_Ctrl = *LCD_Ctrl_PROXY;
	
	*LCD_Data = cmd;
	
	*LCD_Ctrl_PROXY = SetBit(*LCD_Ctrl_PROXY,LCD_E, 1);
	*LCD_Ctrl = *LCD_Ctrl_PROXY;
}
void LCD_WriteCmdEnd() {
	*LCD_Ctrl_PROXY = SetBit(*LCD_Ctrl_PROXY,LCD_E, 0);
	*LCD_Ctrl = *LCD_Ctrl_PROXY;
}
void LCD_WriteDataStart(unsigned char Data) {
	*LCD_Ctrl_PROXY = SetBit(*LCD_Ctrl_PROXY,LCD_RS,1);
	*LCD_Ctrl = *LCD_Ctrl_PROXY;
...
void LCD_Cursor(unsigned char column ) {
	...
	LCD_WriteCmdStart(0xB0 +column); // IEEE change this value to 0xBF+column // was B8
}
}
